...
namespace GameSolver { namespace Connect4 {
  ...
  class Position {
    public:
      ...
      static const int WIDTH = 20;  // width of the board
      static const int HEIGHT = 20; // height of the board
//      static_assert(WIDTH < 10, "Board's width must be less than 10");
      ...
      bool isWinningMove(int col) const 
      {
        ...
        if(height[col] >= 4 
            ...
            && board[col][height[col]-4] == current_player)
          return true;
        ...
        for(int dy = -1; dy <=1; dy++) {    // Iterate on horizontal (dy = 0) or two diagonal directions (dy = -1 or dy = 1).
          ...
          if(nb >= 4) return true; // there is an alignment of at least 4 other stones of the current user 
